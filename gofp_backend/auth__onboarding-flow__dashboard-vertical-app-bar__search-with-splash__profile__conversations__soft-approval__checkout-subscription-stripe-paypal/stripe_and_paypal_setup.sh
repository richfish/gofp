...
cd $APP_NAME
rails g model Order name email user:references stripe_token amount_cents:integer stripe_customer_hash paypal_identifier paypal_payer_id paypal:boolean stripe:boolean first_name last_name fee_cents:integer unsubscribed:boolean
rake db:migrate
cd ..
