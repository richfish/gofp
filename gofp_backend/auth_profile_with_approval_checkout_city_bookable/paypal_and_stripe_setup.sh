...
cd $APP_NAME
rails g model Order name email stripe_token amount_cents:integer stripe_customer_hash paypal_identifier paypal_payer_id paypal:boolean stripe:boolean order_key first_name last_name fee_cents:integer account:references
rake db:migrate
cd ..
...
cp paypal_and_stipe/create_refund_admin.haml $APP_NAME/admin/views/create_refund.haml
