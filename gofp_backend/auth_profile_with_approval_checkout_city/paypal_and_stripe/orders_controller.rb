class OrdersController < ApplicationController
  ...
  def create
    return unless validate_and_set_email_and_name_attributes()
    ...
    charge = Stripe::Charge.create(
      ...
      :description => "OfflineDating",
      :currency    => 'usd'
    )
    @order = Order.create(booking: @booking, stripe: true, paypal: false, stripe_customer_hash: customer.id, stripe_token: stripe_token,
  ...
  #   return unless verify_security_key(order.order_key)
  #   unless params[:password] == params[:password_confirmation]
  #     return render json: { :error => 'password_error'}, status: 400
  #   end
  #   if User.find_by_email(order.email)
  #     return render json: { :error => "existing_user_error" }, status: 400
  #   end
  #
  #   CustomerAssociationManager.convert_proto_user_to_user(order, params[:password])
  ...
  # def check_expired
  #   unless (Booking.find_by_id(params[:booking_id]) && !Booking.find_by_id(params[:booking_id]).expired)
  #     artist = User.find_by_id(params[:user_id])
  #     if artist
  #       flash[:warn] = "Bookings expire after an hour. Please re-book."
