  ...
  def avatar_field(avatar, f, opts={})
    ...
    ret = <<-END
      <div class='form-group margin-bottom-25px'>
        #{image_tag URI.unescape(avatar.url(:medium)), style: 'margin-bottom:10px'}
        <div></div>
        #{f.file_field :avatar, class: 'input-lg form-control', :'data-url' => opts[:image_url], style: ('display:none' if has_avatar)}
